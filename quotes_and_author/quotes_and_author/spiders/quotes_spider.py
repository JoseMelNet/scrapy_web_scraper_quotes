...
AUTHORS_XPATH= '//small[@class="author"]/text()'
...
class QuotesSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'MEMUSAGE_NOTIFY_MAIL': ['josemelnet@gmail.com'],
        'ROBOTSTXT_OBEY': True,
        'USER_AGENT': 'PlatziMasterScrapy',
    ...
    def parse_only_quotes(self, response, **kwargs):
        ...
        authors_actual_page = response.xpath(AUTHORS_XPATH).getall()
        list_quotes.extend(quotes_actual_page)       # Append the quotes in actual page
        list_authors.extend(authors_actual_page)     # Append the authors in actual page
        ...
        if next_page_button_link:
            yield response.follow(next_page_button_link, callback=self.parse_only_quotes,
                                  cb_kwargs={'quotes': list_quotes, 'authors': list_authors})
    ...
    def _parse(self, response):
        ...
        authors = response.xpath(AUTHORS_XPATH).getall() # authors at home
